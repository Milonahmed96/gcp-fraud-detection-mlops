...
def isolate_env(monkeypatch: pytest.MonkeyPatch, tmp_path):
    """Strip GCP vars and point dotenv at an empty file.

    Without this a developer's real `.env` would leak into the test run and mask
    the missing-key failures we are trying to assert.
    """
